...
const TableManager = require('../database/table-manager') // eslint-disable-line no-unused-vars
...
class CommandManager {
   ...
   * @return {Promise<void>}
   */
  addCommand (command, path, isDefault = false) {
    return new Promise(async (resolve, reject) => {
      command.path = path
      if (isDefault) command.isDefault = true
    ...
    return new Promise(async (resolve, reject) => {
      ...
      for (let dir of this.loadDirs) {
        ...
        for (let commandFunc in commandFuncs) {
          ...
          this.addCommand(command, `${dir.path}/${commandFunc}.js`, dir.defaultCommands).then(() => {
            loaded++
            totalLoaded += command.getAllCommands().length
            if (loaded >= unloaded) {
              this.logger.info(`Loaded ${totalLoaded} command${totalLoaded === 1 ? '' : 's'} from ${dir.path}`)
              loadedDirs++
  ...
  reloadCommand (name) {
    let reload = (path, isDefault) => {
      return new Promise(async (resolve, reject) => {
        ...
        this.addCommand(command, path, isDefault).then(() => {
          this.logger.info(`Successfully reloaded file for command '${command.name}'`)
          resolve()
        }, err => {
          this.logger.error(`Failed to reload file for command '${command.name}': ${err.stack}`)
          reject(err)
        })
      })
    }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let result = this.findCommand(name, false)
      if (result !== null) {
        let path = result.command.path
        return reload(path, result.command.isDefault).then(resolve, reject)
      } else {
        for (let dir of this.loadDirs) {
          ...
          if (fs.existsSync(path)) {
            try {
              /** @type {Command} */
              return reload(path, dir.defaultCommands).then(resolve, reject)
            } catch (ex) {
              return reject(new Error(`Could not reload command '${name}':\n\`\`\`js\n${ex.stack}\n\`\`\``))
            }
          }
        }
  ...
  runResult (result, msg, sudo = false) {
    return new Promise(async (resolve, reject) => {
      ...
      if (result.error) {
        if (!silent) this.bot.client.createMessage(msg.channel.id, result.data)
        ...
        if (sudo || await this.checkPerms(command, msg.author.id)) {
          ...
          if (!this.silent) this.bot.client.createMessage(msg.channel.id, ':lock: You do not have permission to use this command')
        }
      }
      resolve()
    })
   ...
   * @return {CommandResult}
   */
  parseContent (content, commands = this.commands, parent) {
    /**
     * @param {Command} command
     * @param {string} content
     * @return {CommandResult}
