...
export class CommandPermissionManager {
  ...
  public getPermissions(
    ignoreNone: boolean = false,
    ignoreMode: boolean = false
  ): Promise<string[]> {
            ...
            resolve(
              thisPerms == null
                ...
                : parentPerms == null
                  ? null
                  : parentPerms.filter(
                      perm => !thisPerms.some(perm2 => perm2 === perm)
                    )
            );
            break;
          default:
  ...
  public setPermissions(
    ...
    return this.table.setStringArray(command.getName(), {
      name: CTICols.permissions.name,
      value: permissions
    });
  }

  public getDefaultPermission(
    command: CommandOrGroup,
    ignoreNone: boolean = false,
  ...
  public setDefaultPermission(
    ...
    return this.table.setBoolean(command.getName(), {
  ...
  public setPermMode(command: CommandOrGroup, permMode: string): Promise<void> {
    return this.table.set(command.getName(), {
...
export default class PermissionManager {
  ...
  public checkPerms(command: CommandOrGroup, userId: string): Promise<boolean> {
    return new Promise(async (resolve, reject) => {
      ...
      const isPrivate =
        (await this.bot.table.getBoolean("private", "value", true)) &&
        command.getName() !== "sudo";
      if (commandTags.find(tag => userTags.some(tag2 => tag2 === tag))) {
        if (!((await this.getDefaultPermission(command, true)) && !isPrivate)) {
          return resolve(true);
        } else {
          resolve(false);
        }
